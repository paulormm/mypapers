...
\begin{resumo} 
Um número significante de projetos de Software Livre tem sido amplamente utilizado 
e considerado de sucesso. Entretanto, existe um número maior de projetos que não 
ultrapassam o passo inicial de construção de uma comunidade usuários e desenvolvedores. 
%
Neste estudo, investigamos a existência de uma correlação entre um 
selecionado conjunto de métricas de código-fonte e a atratividade de projetos de
...
do SourceForge.net. 
%
Os resultados indicaram que a atratividade está correlacionada com algumas 
métricas de código-fonte. Isso significa que, independente dos interesses individuais, 
os usuários e desenvolvedores parecem também serem influenciados pelas características
do código-fonte ao adotar e contribuir com um software. 
%
Dessa forma, os líderes de projetos devem monitorar as métricas de código-fonte, 
mais específicamente um conjunto de métricas calculadas de forma objetivas. 
Isso porque, elas podem ter uma influência positiva em suas chances de formar 
uma comunidade de contribuidores em torno do software.
\end{resumo}
...
in November of 2009, from Sourceforge.net, one of the most popular source code
...
Typical definitions for quality assessment are hard to apply to Free Software projects,
as described in the ISO 9126 \cite{ISO9126}.
%
Since, Free Software projects possess a varying number of voluntary contributors 
and many of the definitions present in the ISO are not performed explicitly \cite{Michlmayr2005}.
%
On the other hand, some aspects of product quality can be evaluated by using
software metrics, and some of these metrics can be collected automatically.
%
...
LOC calculates all lines that are not lines of comments and blanks.

Another useful metric for software size is the \emph{number of modules}, 
which is somewhat less influenced by programming languages and 
...
\emph{Number of Methods} is used to measure the class' size through perspective of 
the operations managed by itself. A class should not have an excessive number of operations.
%
The average of this metric is used to help identify the reuse potential of a class. 
In general, classes with a large number of operations have a greater difficult 
to be reused \cite{Lorenz94}.
...
communication between classes \cite{Bansiya97}.
% 
Firstly, attributes of a class only can be manipulated by their methods.Therefore, 
a class attribute should be private to avoid high complexity, because it is hard to predict 
the effects when the public attributes are changed. Thus, the optimal number for
this metric is zero. This means the lowest possible number of public attributes and variables.
%
Secondly, methods of a class represent to the size of class' interface. Methods 
are the functions provided by the class. High values for this 
metric indicate that a class has a lot of functions which could be broken.
...
Methods}, originally proposed by Chidamber and Kemerer \cite{Chidamber94}.
...
between these methods, if they invoke one each other and vice-versa.
...
%
In other words, if coupling is high, the software tends to be less flexible and 
more difficult to adapt.
%
An objective metric for measuring coupling is Coupling Between Objects (CBO), 
also proposed by Chidamber and Kemerer. CBO measures how many modules that are being 
...
%
However, they are metrics that must be considered together. When combined, the 
...
Finally, LCOM and CBO were originally proposed with the object-oriented paradigm in mind.
However, if we generalize the concepts of ``class'' and ``method'' to ``module''
and ``function'', we can also apply them to other programming paradigms. Thus, in this
study, we considered LCOM and CBO able to compose our metric of structural complexity.
...
%
Attractiveness is the capacity of attracting users and developers to a project.
%
...
%
Attractiveness can be estimated through the number of people that
...
%
Therefore, before a Free Software project can receive failure reports, bug fixes, 
...
members into a positive feedback loop.

At first, one might question whether the number of downloads and number of members 
at SourceForge.net are the best proxies to number of users and number of 
developers in the project, respectively.
%
However, this study explored a larger number of projects and applied the same criteria on them.
The number of downloads is a generalization on the number of people interested in the software. 
The number of members point to the governance of the project.

Our hypotheses were based on the ideas that most developers of a Free Software project 
are mainly interested in the interaction with quality code, so they can learn 
from it and more easily contribute to its further development.
%
...
We analyzed Free Software projects from SourceForge.net written in C and had been
downloaded at least once. Our goal was to characterize them
with respect to structural complexity, lines of code, number of modules, 
number of downloads, and number of members.
%
The programming language and metrics were selected according to
\texttt{Analizo's} features, our source code analysis tool.

\subsection{Analizo Tool}

\texttt{Analizo}\footnote{\url{http://softwarelivre.org/mezuro/analizo}}
is a multi-language source code analysis tool. It parses source codes
into multiple languages and reports useful information about it, such as module 
dependency and source code metrics.

A basic requirement of our source code analysis tool is its ability to analyze 
source codes written in several languages. 
%
Most existing tools use object codes to extract data. However, \texttt{Analizo} 
was developed to support source code parsers, since using object codes are hard 
to analyze projects that do not compile, due to failures in either the codes or 
in its dependencies \cite{hassan05}.

\texttt{Analizo} uses \texttt{Doxyparse} \footnote{\url{http://softwarelivre.org/mezuro/doxyparse}}
to parse the source code, which is a multi-language source code parser based on 
Doxygen's internals.
%
In theory, it supports parsing all the languages supported 
by Doxygen \footnote{\url{http://www.doxygen.org}},
but it is currently tested and expected to work with C, C++ and Java source codes.

When we downloaded the source codes from Sourceforge.net's projects and analyzed them,
\texttt{Analizo} was tested to work well with C, providing the following metrics:
%
...
are metrics that support exclusively source code attributes on Object-Oriented paradigm.
%
In any case, in this first study, we did not try an adaptation because we did not want 
...
measure number of lines, consequently, they are similar. 
%
...
%
In this study, we used its data available in a database managed by 
the University of Notre Dame \footnote{\url{http://www.nd.edu/~oss/Data/data.html}} 
and another database provide by project FLOSSMole \footnote{\url{http://flossmole.org/}}. 
%
We accessed these databases in November of 2009 and collected data about 
all the projects that matched the following criteria:
%
\begin{itemize}
\item \emph{written in C}, because \texttt{Analizo} was limited to analyzing C
source code at that time and because there are few academic works that evaluate 
...
i.e. only a register or initial project, or 
...
from the University of Notre Dame and FLOSSMole on the one hand and the source code metrics calculated by
\texttt{Analizo} on the other hand) so that we could perform the needed statistical
...
several subjective matters such as favorite programming language/style and 
potential user community size, for example.

In this paper, we investigated whether some attributes obtained through 
source code quality metrics may influence the attractiveness of a Free 
...
\emph{H2: Free Software projects with higher total lines of code have
higher attractiveness.}
%
Lines of code may reflect the amount of features of the project. Therefore, 
projects with more lines of code will usually attract more users 
...
higher attractiveness.}
%
The number of modules may indicate the project' size and its possibility
of working in independent modules. Project' size also represents
opportunities to contribute. This attracts more members, who can
write more features and fix more bugs, which would also attract other users.
...
%
Before this model be applied, we analyzed the descriptive statistics' values 
...
Firstly, we analyzed the variables in their natural form to verify their
distribution. 
%
Skewness' and Kurtosis' probability distribution showed high values. Therefore,
our variables were transformed into logarithm values for linearisation, 
which reduced Skewness' and Kurtosis' values \cite{Crowston2002}.
%
The mean and standard deviation of the logarithm values are also presented 
...
Secondly, we tested our hypotheses according to defined multiple regression model.
%
In table \ref{table:regression} is summarized the regression results based on the  
linearized Pearson's correlations' values.
%
...
%
The $R$-value represents amount of the dependent variables that can be 
explained by that set of independent variables.
%
In our analysis, $R$-value indicated that source code metrics explain 18\% ($R^2 = 0.180$) of
the number of downloads and 12\% ($R^2 = 0.121$) of the number of members.
...
fixes.
%
So, most users may lose interest in the software because another
project may have a greater capacity to meet their needs.
%
A smaller number of users generate less reports could lead to less bug fixes and
...
% H2
The second hypothesis (\emph{H2: Free Software projects with higher total
...
In this context, lines of code is an indication of the amount of
software features.  The more features available, the more users
...
developers are interested in large projects. The $\beta$ coefficient of
...
%
Lines of code represents the amount of features in the project, which
helps to attract both users and developers to the project. When lines of
code is kept constant, different values in number of modules represent
different ways of locating those features over different modules. A
...
modular software.
%
For users, on the other hand, it does not matter whether the software is
modular or not; they are only interested in the provided features.
...
complexity as low as possible and modularity as high as possible.

\section{Related Work}

Large Free Software projects as Debian, GNOME, and KDE have invested in the 
creation of dedicated teams for quality assurance. These efforts involve everything
from removing bugs and obsolete components to the definition of quality standards
and strategies to prevent bugs and improve quality \cite{Michlmayr2005}.
However, many projects do not have the resources to have a dedicated quality
team. 
%
...
%
However,the values of source code metrics were not associated with problems or attractiveness
of Free Software projects.

Stamelos \emph{et al.} \cite{Stamelos2002} presented empirical results on the relationship
between size of the application components and delivered quality
measured through user satisfaction. 
%
Quality characteristics of 100 applications written for Linux were compared to 
...
%
\footnote{ They defined software design quality in terms of 5 Object-Oriented metrics,
...
managerial knowledge to project leaders, pointing them io the right
direction on prioritizing their resources.

Our results indicated that source code size and structural complexity
explain a relevant percentage of Free Software projects' attractiveness.
...
completely. Nevertheless, this study was able to explain  18\% of software 
downloads (users) and 12\% of project members (developers), through a set of 
four source code metrics.

In this study, lines of code (LOC), representing the software features,
has a great effect on the number of project users. Additionally, before a member
becomes a project developer, he is a user.
%
Our results also indicated that structural complexity has a negative 
influence on projects' attractiveness.
%
Therefore, a project will face greater difficulties to grow without observing
some source code attributes such as cohesion, coupling, and number of modules,
which favors developers contributions such as new features and bug fixes.
...
around their software, enhancing its quality further.
Thus, projects should grow managing their complexity, and keeping new
...
apparently be able to explain roughly 40\% of attractiveness variance.
%
However, to statistically include variables in an equation is not 
...
The authors of this paper are supported by CNPQ, FAPESP, and Qualipso project. 
This research is also part of CCSL-USP (Free Software Competence Center). 
The authors would like to thank Claudia Melo, João Miranda, Beraldo Leal 
and Nelson Lago for their collaborations.
