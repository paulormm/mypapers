...
whether attractiveness, the ability of a projet to attract users and
developers, can also be influenced by measurable source code
characteristics. 
...
%
Section \ref{hypotheses} shows our hypotheses and discusses the hypotheses testing
and results. 
%
Section \ref{relatedWork} reviews related work.
%
Conclusions, threats to validity, and future research
directions are discussed in Section \ref{conclusion}.
...
%
We used on a statistical model for our study seven metrics selected according to
criteria shown in Section \ref{researchDesign}. The concept of attractiveness 
and its proxies were based on Santos Jr.'s attractiveness model \cite{Santos2010}.
...
%
For our selected metric set, we use the concept of module as a general term for
...
The first of metrics is \emph{Lines of Code} (LOC) that is the most commonly 
used metric to measure software size.
%
...
\texttt{Doxyparse}\footnote{\url{http://softwarelivre.org/mezuro/doxyparse}}, 
...
%
CCCC\footnote{\url{http://cccc.sourceforge.net/}}, 
Cscope\footnote{\url{http://cscope.sourceforge.net/}},
Eclipse-Metrics\footnote{\url{http://metrics.sourceforge.net/}}, 
and Macxim/Spago4Q\footnote{\url{http://qualipso.dscpi.uninsubria.it/macxim/}}.
%
...
proposed as metrics for the object-oriented paradigm in mind.  Nevertheless, if we
generalize the concepts of ``class'' and ``method'' to ``module'' and
``subroutine'', respectively, we can also apply them to other programming
paradigms.
...
%frase errada gramaticalmente; não entendi o significado.
%
In any case, for this first study, we did not try a adaptation
...
and another one provided by the FLOSSMole project\footnote{\url{http://flossmole.org}}. 
%
...
on the Web\footnote{\url{http://ccsl.ime.usp.br/mangue/data}}.

\section{Research hypotheses}
\label{hypotheses}

Attractiveness is about human decisions \cite{Santos2010}. Many factors
may influence an user to download a piece of software, such as the
indication of a friend, a hint from a magazine, a fancy Web site, work
...
complexity constant as new code is incorporated, because developers are
...
straightforward. Accordingly, another future venue to pursue is  
to identify the interaction between that set of variables with the ones reported
in this study.
