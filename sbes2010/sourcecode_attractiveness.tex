...
\author{
  ...
  \begin{affiliation}
    ...
    (CCSL-IME/USP)\\
    \email{\{paulormm,denner,kon\}@ime.usp.br}
  \end{affiliation}
  \and
...
contributors around the software, enabling further enhancement in its quality.
\end{abstract}
...
To us, this means that no more than 6\% of the projects on
SourceForge.net are in conditions to have a healthy community of users
...
%
A number of these factors are highly subjective, such as audience,
community building processes, and the project leaders managerial skills.
%
However, our focus in this study is on objective factors: we investigate
whether attractiveness, the ability of a project to attract users and
developers, can also be influenced by measurable source code characteristics. 
...
%
In this model, he suggested that the success of any project depends on
its level of attractiveness to potential contributors.
%
...
metric and two size metrics play an important role in explaining attractiveness,
here represented by number of downloads and number of project members.

The remainder of this paper is organized as follows:
%
Section \ref{background} presents the theoretical background about source code
...
%
Section \ref{hypotheses} shows our hypotheses and discusses the hypotheses tested
and results. 
%
...
Section \ref{researchDesign}, on a statistical model for our study.
The concept of attractiveness and its proxies were based on Santos Jr.'s
attractiveness model \cite{Santos2010}.

Firstly, some aspects of product quality can be evaluated by using
...
which also contradicts the recommended best practices in object-orientation.
...
nodes are the methods. When two methods use at least an attribute in
...
C and that had been downloaded at least once. Our goal was to characterize them
...
is a multi-language source code analysis tool. Its architecture was designed in
...
For this study, we used NM, NPA, NPM, LCOM and CBO. They are metrics originally 
...
was limited to analyzing C source code and because there are few academic works 
...
we could perform the needed statistical analyses. The complete data set used 
for this study is available on the Web\footnote{\url{ccsl.ime.usp.br/mangue/data}}.

\subsection{Variables}

\TODO{Carlos: verificar se ainda estah confusa esta section}
...
source code attributes that they provide.
...
LOC is a global level metric that include LOC of methods. Because these reasons, 
...
one of them for our statistical analysis.

\TODO{Carlos: preciso de uma referencia para o paragrafo abaixo? Vc tem? 
Eu li isto na internet. O Fabio quer essa explicacoes em detalhes sobre as 
escolhas estatisticas porque ele nao sabe, entao os outro tb nao sabem ;)}

Also we analyzed the Spearman and Kendal non-parametric correlations that 
indicate a proportional result on correlation between these metrics.
% 
We chose the Pearson parametric correlation because non-paramteric statistical 
are less powerful. They use only the ordinal position of pairs of scores as 
information in their calculations while a parametric correlation uses information
about the mean and deviation from the mean.
...
Attractiveness is about human decisions \cite{Santos2010}. Many factors
...
%
Lines of code somehow reflect the amount of features of the project and
...
We defined a multiple regression model to explain the relationships
between the
...
%
It shows the sample size (N) for each, natural values of minimum, maximum, mean, 
...
\TODO{from FK to Carlos: O que eh standardized beta?}
...
\TODO {from FK to Carlos: o paragrafo que explica a Tabela 3 estah ruim. Apenas quem 
jah entende os conceitos de standardized beta, T-value, P-value e R entende 
o que estah escrito. A chance de algum dos revisores do SBES entender esses 
conceitos eh proxima de zero. O paragrafo tem que explicar os conceitos e 
depois tirar as conclusoes a partir dos dados da tabela.}
...
downloads (a proxy for number of users) and 12\% of project members 
(a proxy for number of developers), through a set of four source code metrics. 
%
These statistical results are significant for a social context such as 
...
This study has some limitations that motivate future work.
%
...
\TODO{Carlos: Eu reescrevi partes do paragrafo abaixo, por favor verificar}

Finally, we acknowledge that source code metrics are not the only
variables capable of influencing attractiveness.
%
Our previous work has identified that things such as restrictiveness of
