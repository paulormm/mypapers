...
\author{\IEEEauthorblockN{Paulo Meirelles, Carlos Santos Jr., Jo\~ao Miranda, Fabio Kon}
	\IEEEauthorblockA{FLOSS Competence Center\\
...
coupling nor cohesion are related to a software maintenance effort.
%
Both must be considered together. When combined, the product of coupling and
cohesion as a metric is positively correlated to the maintenance effort.
...
\begin{figure*}[!t]
\centering
\includegraphics[scale=.34]{figure/attractiveness3}
...
%
In short, we propose a new element that can explain attractiveness partially.
%
...
%
Although we recognize that using these proxies is a limitation, we did not find
in previous works better proxies that could represent more faithfully the number
of users and number of developers in a large sample of projects.
...
the languages supported by \texttt{Doxygen}\footnote{\url{doxygen.org}}.
% 
So, up to the moment, it has been tested only with C, C++, and Java source code, 
...
(Average) Coupling Between Objects      & 0.0015 & 711.50         & 2.26       & 9.04           & 0.35 & 0.98 \\ \hline

(Average) Lack of Cohesion on Methods   & 0.0004 & 262.00         & 4.77       & 12.00          & 1.01 & 1.09 \\ \hline
...
(Total) Lines of Code                   & 11     & 2,983,103.00   & 17,722.23  & 91,614.70      & 8.28 & 1.58 \\ \hline
...
(Total) Number of Downloads             & 6      & 941,498,760.00 & 956,674.26 & 17,760,732.37 & 8.20 & 2.66 \\ \hline
...
the research that addresses it, and our previous experience with
analysis of Free Software written in C \cite{terceiro2009}, we
chose to focus the analysis in this work to such projects as well.
%
...
errors in the source code (e.g., syntax errors);

\item Cross-join the two datasets. Finally the two datasets --
...
%
Section~\ref{variables} discusses in detail how we selected the variables
...
%
Thereby, we observed that the Skewness and Kurtosis probability distribution
...
and average CBO.
%
To be able to apply to the procedural paradigm of the C language some metrics
that area widely-used in the literature such as NOF, NPV, NPF, LCOM4, and CBO,
we assumed a mapping of the object concepts of ``class'' and ``method'' to the
C concepts of ``source file'' and ``function''.
%% by fabio kon

%
In this first study, we limited the scope of metrics used to be able to reach
a simple yet comprehensive model to relate source code and attractiveness.
%
Thus, the ACC, AMZ\_Size, COF, DIT, NOC, NOV, and RFC metrics were left out of
...
Nevertheless, in the first stage of our statistical analysis,
the LOC, NOF, NPV and NPF metrics showed a high correlation between each other,
according to Pearson's parametric correlations as shown by the bold numbers
in Table~\ref{pearson}.
%
...
(see Table~\ref{spearman} and Table~\ref{kendall}, respectively)
given that some of our variables are not normally distributed. 
%
...
According to the analysis described above, we ended up considering LOC and NM 
...
%
In conclusion, LOC and NM were collected since they measure different kinds of
size metrics, more or less influenced by programming languages and coding
styles, respectively.
%
Finally, to obtain the value of our structural complexity metric (SC),
explained in Section~\ref{background}, CBO and LCOM4 were multiplied.
%
...
%
Consequently, the SC, LOC and NM metrics, which represent the source code attributes,
...
H1 -- \emph{Free Software projects with higher structural complexity have
...
H2 -- \emph{Free Software projects with more lines of code have
...
H3 --  \emph{Free Software projects with a higher number of modules have
higher attractiveness.}
%
...
%
With the results in hand, we selected the variables of our regression model
according to our scope definition, the analysis of the Pearson parametric
...
as expected. Noteworthy is that the T-test and P (probability) values represent
whether a source code metric is a statistically significant predictor or
influencer of attractiveness indicators.
%
...
%
So, roughly speaking, an R-squared of 20 percent indicates that a set of
...
number of members. 
%
These are significant values for the social context that an
...
However, when lines of code is kept constant, different values in the number of
modules represent different ways of organizing these features over different modules.
%
...
the present study.
%
Therefore, a straightforward comparison between their study and ours is not
so simple.
...
governments, and companies) the importance to monitor metrics such as LCOM4 and CBO
together with NM and LOC, thereby increasing their chances of forming a community
of contributors around their software, further enhancing its quality.
%
Thus, projects should grow managing their complexity, keeping the new
members willingness to contribute.

Our study differs from related work because we analyzed a large sample of Free
Software projects.
%
Table \ref {table:statistics} shows how diverse our sample of 6,773 projects is.
%
There are projects with thousands of modules
(7,177 -- Broadcom replacement firmware\footnote{\url{sourceforge.net/projects/newbroadcom}}),
%
millions of lines of code 
(2,983,103 -- Broadcom replacement firmware), %% eh o mesmo que o anterior, eu nao errei ;)
%
large structural complexity
(4,940 -- pyCDK\footnote{\url{sourceforge.net/projects/pycdk}}),
%
several hundred members
(288 -- TinyOS\footnote{\url{sourceforge.net/projects/tinyos}}),
%
and hundreds of millions of downloads
(941,498,760 -- MinGW: Minimalist GNU for Windows\footnote{\url{sourceforge.net/projects/mingw}}).
%
...
independent from the source code metrics we studied here, roughly 40\% of
attractiveness variance would be then explained.
%
However, including variables in an equation in a statistically sound manner
is not a trivial task.
%
Accordingly, there is a need to further identify the interaction between that
set of variables with the ones reported in this study.
